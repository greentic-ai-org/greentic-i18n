...
use crate::json_map::parse_json_map;
use std::env;
...
fn load_catalog(locale: &str) -> Result<JsonMap, String> {
    ...
    parse_json_map(raw).map_err(|err| format!("failed loading locale `{locale}`: {err}"))
}
